...
import namedEntity.Classes.ClassesUtils;
...
public class Article {
	...
	public void computeNamedEntities(Heuristic h) {
		...
		ClassesUtils u = new ClassesUtils();
		...
		for (String s : text.split(" ")) {
			if (h.isEntity(s)) {
				...
				if (ne == null) {
					NamedEntity e = u.instanceEntity(h.getCategory(s), s);
					e.setTopic(h.getTopic(s));
					this.namedEntityList.add(e);
				} else {
					ne.incFrequency();
				}
			}
	...
	public void prettyPrintV2() {
		System.out.println("╔════════════════════════════════════════════════════════════════════════════════════╗");

		// TITLE
		wrapAndPrint("📰 Title: ", this.getTitle());

		System.out.println("╠════════════════════════════════════════════════════════════════════════════════════╣");

		// DATE (no se parte porque siempre es corto)
		System.out.printf("║ %-80s ║\n", "📅 Date: " + this.getPublicationDate());

		// LINK
		wrapAndPrint("🔗 Link: ", this.getLink());

		System.out.println("╠════════════════════════════════════════════════════════════════════════════════════╣");

		// TEXT
		wrapAndPrint("📝 Text: ", this.getText());

		System.out.println("╚════════════════════════════════════════════════════════════════════════════════════╝");
	}

	private void wrapAndPrint(String label, String content) {
		int maxWidth = 80;
